...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train snow overlay head with full-res noise mask.")
    ...
    parser.add_argument("--overlay-hidden", type=int, default=64)
    parser.add_argument("--overlay-scale", type=float, default=0.5)
    parser.add_argument("--snow-weight", type=float, default=8.0)
...
class SnowOverlayNet(nn.Module):
    ...
    def forward(self, recon: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        x = torch.cat([recon, mask], dim=1)
        return self.net(x)
...
def main() -> None:
    ...
    overlay_net = SnowOverlayNet(hidden=args.overlay_hidden).to(args.device)

    if args.train_decoder:
        for p in vqvae.decoder.parameters():
            p.requires_grad = True
        train_params = list(vqvae.decoder.parameters()) + list(overlay_net.parameters())
    else:
        for p in vqvae.decoder.parameters():
            p.requires_grad = False
        train_params = list(overlay_net.parameters())
    ...
    print(f"Start snow HR training | device={args.device} | train_decoder={args.train_decoder}")
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(dataloader):
            ...
            with torch.amp.autocast("cuda", enabled=args.use_amp, dtype=amp_dtype):
                if args.train_decoder:
                    ...
                    recon = vqvae.decoder(quantized).detach()
                overlay = overlay_net(recon, snow_mask)
                overlay = torch.tanh(overlay) * args.overlay_scale
                output = torch.clamp(recon + overlay, 0.0, 1.0)
                ...
                if args.laplacian_weight > 0:
                    hf_loss = F.l1_loss(laplacian_filter(output), laplacian_filter(target))
        ...
        if (epoch + 1) % args.save_every == 0:
            ckpt_path = os.path.join(args.out_dir, f"decoder_snow_noise_hr_ep{epoch}.pth")
            payload = {
                "overlay": overlay_net.state_dict(),
        ...
        with torch.no_grad():
            ...
            preview = torch.cat([base, target, output, snow_vis])
            save_path = os.path.join(args.out_dir, f"preview_snow_noise_hr_ep{epoch}.png")
